...
本分支支持 Linux 源码安装及 Ubuntu/Debian x64 的 `.deb` 包。WorkBuddy 官方客户端需单独安装；
WorkDaddy 通过 CDP 连接它，不修改官方程序。

PR #180 的 Linux 适配曾在 WorkBuddy 5.5.4 + Node 22.22.2 上实测；新的 `.deb` 内置 Node 22.23.1，仍需在 Linux 构建机执行实际安装验收。
...
### Debian 包（Ubuntu/Debian x64）

在 Linux x86_64 构建机上从本仓库运行：

```bash
bash scripts/build-linux-deb.sh
sudo apt install ./release/linux/WorkDaddy_1.2.65_amd64.deb
```

包内置校验过的 Node 22 和 `ws`，安装在 `/opt/workdaddy`，桌面菜单提供 WorkDaddy 与
WorkDaddy AI 两个入口。首次点击会以当前用户准备备份目录并启用 CDP；若需要重启正在运行的
WorkBuddy，会先弹出确认。国际版使用已有的隔离 HOME/启动器配置；没有安装官方客户端时会
提示找不到对应可执行文件。安装包不在 root 权限下运行配置脚本，也不会自动启用开机服务。

构建脚本允许通过 `WORKDADDY_BUILD_VERSION` 指定发行版本（只修改暂存包内版本）；
离线构建可提供 `WORKDADDY_NODE_ARCHIVE` 和 `WORKDADDY_WS_ARCHIVE`，仍须通过固定 SHA-256 校验。
要在 Linux 实机运行 `node --test test/*.test.js`，并测试两个入口的注入、账号/会话读取与卸载。
卸载 `sudo apt remove workdaddy` 只移除程序文件，不删除用户的账号备份。
此包没有自动更新，升级时安装新版 `.deb`。
若从不同 Node 路径运行的源码版升级，安装后的首次启动会拒绝按旧锁文件 PID 停止
无法验证身份的 daemon；先手动确认并停止旧进程，再重新打开桌面入口。
...
- 面板内「检查更新」在 Linux 下会提示不支持自动更新；发行包升级请安装新版 `.deb`，源码安装则更新源码。
