...
- daemon 在发送前读取遥测设置：About 页「发送错误诊断」和 `WORKDADDY_TELEMETRY` 的现有优先级不变，无绕过开关的采集选项。关闭开关会取消进行中的请求，但无法撤回已经送达的请求。
...
统计用户数是自该版本启用后被服务端观测到的安装数；不代表自然人数，关闭遥测或未升级的用户不会计入。同一电脑保留共享数据重装仍算一个安装；更换电脑或删除共享数据则算新安装。
